...
func UsageCommands() string {
	return `calc (add|sub|mul|divide)
`
}
...
func ParseEndpoint(
	...
	var (
		...
		calcMulFlags = flag.NewFlagSet("mul", flag.ExitOnError)
		calcMulAFlag = calcMulFlags.String("a", "REQUIRED", "Left operand")
		calcMulBFlag = calcMulFlags.String("b", "REQUIRED", "Right operand")
	...
	calcMulFlags.Usage = calcMulUsage
	...
	)
	{
		...
		case "calc":
			...
			case "mul":
				endpoint = c.Mul()
				data, err = calcc.BuildMulPayload(*calcMulAFlag, *calcMulBFlag)
			case "divide":
				endpoint = c.Divide()
...
func calcMulUsage() {
	fmt.Fprintf(os.Stderr, `%[1]s [flags] calc mul -a INT -b INT
...
Example:
    %[1]s calc mul --a 7309877832173772408 --b 3237209857320107068
`, os.Args[0])
}
